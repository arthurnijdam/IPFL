...
class P2P_AFPL():
        ...
        for idx, i in enumerate(selected_clients):
            ...
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(dataloader_test):
                    ...
                    loss_test += F.nll_loss(output, target)
                losses += loss_test / self.len_test[str(i)]
        self.current_test_loss[str(0)] = losses
        if self.current_test_loss[str(0)] < self.best_test_loss[str(0)]:
            torch.save(self.client_models[str(0)].state_dict(),
                       os.path.join(save_dir, 'model', 'best_model' + str(0) + '.pt'))
            self.best_test_loss[str(0)] = self.current_test_loss[str(0)]

        #losses += loss_test / self.len_test[str(0)]
        loss_test2 = 0
        for idx, i in enumerate(selected_clients):
            self.client_models[str(0)].eval()
            with torch.no_grad():
    ...
    def loop(self, epochs, p2p, experiment_name):
        ...
        for i in range(epochs):
            ...
            if self.test == 'bandits':
                ...
                for client in range(self.total_clients):
                    loss_test2, loss_train2, selected_clients2 = self.bandits(client, n_bandits_UCB)
                    losses2 += loss_test2
            ...
            if self.test != 'local' and self.test != 'centralized':
                ...
                fname = os.path.join(
                    '/mimer/NOBACKUP/groups/snic2022-22-122/arthur/code/Federated_Averaging/checkpoints_bandits',
                    experiment_name, 'losses_test.txt')
                np.savetxt(fname, loss_tests2)
                fname = os.path.join(
                    '/mimer/NOBACKUP/groups/snic2022-22-122/arthur/code/Federated_Averaging/checkpoints_bandits',
                    experiment_name, 'losses_train.txt')
        ...
        fname = os.path.join(
            '/mimer/NOBACKUP/groups/snic2022-22-122/arthur/code/Federated_Averaging/checkpoints_bandits',
            experiment_name, 'accuracies.txt')
        ...
        plt.savefig(
            os.path.join('/mimer/NOBACKUP/groups/snic2022-22-122/arthur/code/Federated_Averaging/checkpoints_bandits',
                         experiment_name, 'loss_curve.png'))
        plt.clf()
        ...
        plt.savefig(
            os.path.join('/mimer/NOBACKUP/groups/snic2022-22-122/arthur/code/Federated_Averaging/checkpoints_bandits',
                         experiment_name, 'accuracy_progression.png'))
        return accuracies
