...
def add_dataset(patient_id, dataset, DATASET_PATH):
    ...
    assert torch.equal(dataset["train_y"],
                       dataset_other["train_y"]), "Training ground truths are different. Possibly shuffled differently."
    assert torch.equal(dataset["val_y"],
                       dataset_other["val_y"]), "Validation ground truths are different. Possibly shuffled differently."
    assert torch.equal(dataset["test_y"],
                       dataset_other["test_y"]), "Test ground truths are different. Possibly shuffled differently."
...
def get_error_one_patient(S, F, y=None, as_energy=False):
    ...
    assert F.shape[1] == S.shape[
        1], f"F and S can't be matrix multiplied. Provide S as a matrix with shape (N x {F.shape[1]})."
...
def generate_dictionary(S, num_atoms=20, delta_err=None, max_it=100, max_admm=100, calc_err=False, seed=0,
                        printing=False):
    """
        ...
        if calc_err:
            ...
            if k > 1 and delta_err is not None and np.abs(E[k - 1] - E[k]) < E[k - 1] * delta_err:
                if printing:
                    ...
                    print(
                        f"Stopping early. Abs error diff: {np.abs(E[k - 1] - E[k]):.2e}, Threshold: {E[k - 1] * delta_err:.2e}")
                k = k + 1
                return D, X, E[:k]
